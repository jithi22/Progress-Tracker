...
export var tp = {
  ...
  init : () => {
    ...
    `<div id="tp-box">
      <div id="tp-date-wrap">
        <input type="datetime-local" id="tp-date" class="tp-date"/>
      </div>
      ...
      <button id="tp-set">Log</button>
      <div id="tp-info-wrap">
        <input type="text" id="tp-info" placeholder="Enter info" />
      </div>  
    </div>`;
  ...
  spin : (direction, segment) => {
    // console.log('spinn clickeddd', direction, segment)
    // (B1) CLEAR TIMER
    ...
    //   console.log('next',next)
      ...
      // console.log( tp["h"+segment],  tp["h"+segment].innerHTML, next)
  ...
  show : instance => {
    ...
    // (F2) SET DATE AND TIME TO NOW BY DEFAULT
    let now = new Date();
    let dateTimeLocal = new Date(now.getTime() - (now.getTimezoneOffset() * 60000)).toISOString().slice(0,16);
    document.getElementById('tp-date').value = dateTimeLocal;
    tp.setfield.value = "0000"
  ...
  set : () => {
    // (E1) TIME TO FIELD
    if (tp.set24) {
      if(tp.hhr.innerHTML === '00' && tp.hmin.innerHTML === '00'){
        alert('Please Log the Study Time')
        return
      }
